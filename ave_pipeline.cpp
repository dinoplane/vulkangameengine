...
namespace ave{
    ...
    std::vector<char> AvePipeline::readFile(const std::string& filepath){
        ...
        size_t fileSize = static_cast<size_t>(file.tellg()); // seeked to end to get file size
    ...
    void AvePipeline::createGraphicsPipeline(const std::string& vertFilePath, const std::string& fragFilePath, const PipelineConfigInfo& configInfo){
        ...
        std::cout << "Vtx Shader Size: " << vertCode.size() << "\n";
        std::cout << "Frg Shader Size: " << fragCode.size() << "\n";
        ...
        auto bindingDescriptions = AveModel::Vertex::getBindingDescriptions();
        auto attributeDescriptions = AveModel::Vertex::getAttributeDescriptions();
    ...
    void AvePipeline::createShaderModule(const std::vector<char>& shaderCode, VkShaderModule* shaderModule){
        ...
        createInfo.pCode = reinterpret_cast<const u_int32_t*>(shaderCode.data()); // Vector ensures that chars are aligned ()
    ...
    PipelineConfigInfo AvePipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height){
        ...
        configInfo.scissor.extent = {width, static_cast<uint32_t>(height)};
