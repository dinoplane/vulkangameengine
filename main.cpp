...
private:
    ...
    void cleanupSwapChain() {
        for (size_t i = 0; i < swapChainFramebuffers.size(); i++) {
            vkDestroyFramebuffer(aveDevice.device(), swapChainFramebuffers[i], nullptr);
        }

        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            vkDestroyImageView(aveDevice.device(), swapChainImageViews[i], nullptr);
        }
    ...
    void drawFrame() {
        vkWaitForFences(aveDevice.device(), 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);

        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(aveDevice.device(), swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
        ...
        if (vkQueueSubmit(aveDevice.graphicsQueue(), 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
        ...
        result = vkQueuePresentKHR(aveDevice.presentQueue(), &presentInfo);
